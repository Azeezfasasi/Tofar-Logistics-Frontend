        ...
        <nav className="hidden lg:flex flex-row items-center space-x-4">
          {navLinks.map((link, index) => {
            ...
            if (link.submenus) {
              ...
              >
                <span className="text-gray-700 hover:text-blue-600 font-medium cursor-pointer transition-colors">
                    ...
                    <div className="absolute top-full left-0 mt-0 w-48 bg-white border border-gray-200 rounded-md shadow-lg z-50">
                      {link.submenus.map((submenu) => (
                        <Link
                          key={submenu.href}
                          to={submenu.href}
                          className="block px-4 py-2 text-gray-700 hover:bg-blue-100 font-semibold transition-colors"
                  ...
                  return (
                    <div key={index}>
                      <button
                        ...
                        className="w-full text-left text-gray-700 hover:text-blue-600 font-medium text-lg transition-colors focus:outline-none"
                      >
                        {link.label} <ArrowDownLineIcon />
                      </button>
                      {aboutDropdownOpen && (
                        <div className="flex flex-col space-y-2 pl-4 mt-2">
                          {link.submenus.map((submenu) => (
                            <Link
                              key={submenu.href}
                              to={submenu.href}
                              className="text-gray-600 hover:text-blue-600 font-semibold text-base transition-colors"
                              onClick={() => {
                                  setMenuOpen(false);
